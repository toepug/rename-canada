...
1. **Header:** [fictional NDRS crest] + dual-language org signature side by side ("National Designation Renewal Secretariat / Secrétariat du renouvellement des désignations nationales"), FR toggle top-right, full-width "Search NDRS.ca" bar with dark search button, dark **MENU ▾** dropdown button on its own row
2. **Breadcrumbs:** small chevron-separated trail (`NDRS.ca > Programs > Canada Name Transition Pilot Program`)
...
- Crest built and integrated (2026-07-07, see §10): a maple leaf mid-dissolve into pixels, per `crest/CREST_BRIEF.md`. Original 5-lobed leaf geometry — pointed lobes, wide soft sinuses, thin tapered petiole — built from a polar radius function with a √-falloff bump profile (steep at each peak, flat near the base), sampled and connected with a Catmull-Rom spline. Verified distinct from the flag's 11-point leaf via `/public/crest/comparison.png`. Three files in `/public/crest/`: `crest-signature.svg` (header/favicon), `crest-seal.svg` (double-ring, About page), `crest-watermark.svg` (low-contrast footer). Live in the header (navy), footer (6% opacity watermark), About page (seal near §1.0 Mandate), and the favicon set (fine-particle layer hidden)
